...
let setRoomName = function(dataSanpshot) {
return new Promise (function(resolve, reject){
for(let i =0; i <dataSanpshot.length; i++){
    con.query("Select room_name , id from mrbs_room where id = '" + dataSanpshot[i].room_id + "'", function (err, result_args){
           if (err) reject(err);
           let j =0;
           while(j<re.length){
             roomName = resolved_args[j].room_name;
             if(roomName == null && dataSanpshot[i] == null){
               console.log("roomName or datasnapshoot null");
             } else {
               dataSanpshot[i].room_id = roomName;
               console.log("dataSnapshot :" + dataSanpshot[i])
             }
             j++;
           }
           console.log(dataSanpshot)
           resolve(dataSanpshot)
         });
   }
})
}
fetchRecords("SELECT start_time, end_time, room_id, timestamp, Program, Batch, Course, Semester, Faculty, Division, id FROM mrbs_entry").then(function(resolved_args){
  var datasanpshot = resolved_args

    setRoomName(datasanpshot).then(function(resolve){
    console.log("Returned Promise" + resolve)
...
//    let updated_end_time = dateFormat(new Date(Resultset.end_time * 1000) , "h:MM" );
...
//      let store2_pathRef = firebase.database().ref(path1+ "/Lectures/"+schedule_timestamp);
